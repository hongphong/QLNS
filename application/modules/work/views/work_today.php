<div class="pro-info wrap-main">
	...
	<div class="wrap-work-list">
		<table class="tbl-work-list" width="100%" border="0">
			...
			<tr>
				<td>
					<b class="title-work">Việc mới <font color="red">(<?php print (isset($workNew)) ? count($workNew) : 0; ?>)</font></b>
				</td>
				<td colspan="4"></td>
			</tr>
			<?php
			if (!empty($workNew)) {
				foreach ($workNew as $wn) {
					if (!$wn['name']) continue;
					?>
					<tr>
						<td width="30%">
							<a href="<?php print base_url() . 'iso/step/detail/' . $wn['step_id'] . '?read=1'; ?>">
								<span><?php print $wn['name']; ?></span> [<span><?php print $wn['number']; ?></span>]
							</a>
						</td>
						<td width="20%">
				...
				?>
				<tr>
					<td colspan="5">Không có việc mới</td>
				</tr>
				<?php
			}
			?>
